...
fit.mi.matmor <- with(mice.multi.out, 
                      lm(Maternal_Mortality_Rate ~ -1 + conflict + gdp1000 + OECD + popdens + urban + 
                           agedep + male_edu + temp + rainfall1000 + earthquake + drought + 
                           as.factor(ISOnum) + as.factor(year)))
fit.mi.infmor <- with(mice.multi.out, 
                      lm(Infant_Mortality_Rate ~ -1 + conflict + gdp1000 + OECD + popdens + urban + 
                           agedep + male_edu + temp + rainfall1000 + earthquake + drought + 
                           as.factor(ISOnum) + as.factor(year)))
fit.mi.neomor <- with(mice.multi.out, 
                      lm(NeoNatal_Mortality_Rate ~ -1 + conflict + gdp1000 + OECD + popdens + urban + 
                           agedep + male_edu + temp + rainfall1000 + earthquake + drought + 
                           as.factor(ISOnum) + as.factor(year)))
fit.mi.un5mor <- with(mice.multi.out, 
                      lm(Under5_Mortality_Rate ~ -1 + conflict + gdp1000 + OECD + popdens + urban + 
                           agedep + male_edu + temp + rainfall1000 + earthquake + drought + 
                           as.factor(ISOnum) + as.factor(year)))
